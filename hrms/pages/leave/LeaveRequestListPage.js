...
/**
 * /leave-request-list — employee self-service: raise and track leave requests.
 * Grid: Sl.No | Leave Type | Start Date | End Date | Approval Status | Remarks |
 * Action (captured with 0 rows — headers only, no empty-state text).
 * "New Leave Request" opens the create flow (no inline inputs on the list).
 * Submitted requests surface in the approver worklist /leave-approval.
 */
class LeaveRequestListPage extends BasePage {
  /** @param {import('@playwright/test').Page} page */
  constructor(page) {
    super(page, 'leave-request-list');

    this.newRequestBtn = this.button('New Leave Request');
  }

  /**
   * Open the create UI behind "New Leave Request". Nothing is ever saved.
...
module.exports = { LeaveRequestListPage };
