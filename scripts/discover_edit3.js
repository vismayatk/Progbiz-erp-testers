...
(async () => {
  ...
  try {
    ...
    console.log('my-tasks/Delayed actions:', JSON.stringify(ids));
    if (ids.edit[0]) {
    ...
    if (ids.overview[0]) {
      ...
      console.log('OVERVIEW:', JSON.stringify(await page.evaluate(() => {
        const vis = (e) => e.getClientRects().length > 0;
        return {
          headings: [...document.querySelectorAll('h4,h5,h6,.nav-link,.card-title')].filter(vis).map(e => e.textContent.replace(/\s+/g, ' ').trim()).filter(t => t && t.length < 28).slice(0, 14),
